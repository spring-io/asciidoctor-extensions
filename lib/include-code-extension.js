  ...
  return function () {
    ...
    this.blockMacro(function () {
      ...
      this.process((parent, target, attrs) => {
        ...
        const includeCodeUsePath = doc.getAttribute('include-code-relative-option') !== undefined
        ...
        const relativeIdPath = sectionId ? sectionId.replaceAll('-', '').replaceAll('.', '/') + '/' + target : target
        const srcPath = doc.getAttribute('page-relative-src-path')
        const dirPath = path.dirname(srcPath).replaceAll('-', '')
        const relative = includeCodeUsePath && dirPath !== '.' ? dirPath + '/' + relativeIdPath : relativeIdPath

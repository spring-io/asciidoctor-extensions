...
function addFolds (getContent, { tags: foldTags, imports: foldImports }) {
  ...
  chunks = source.split('\n').reduce((accum, line) => {
    if (foldImports && line.startsWith('import ')) {
      if (!inFoldBlock) {
        accum.push('<span class="fold-block hide-when-folded">')
      } else if (inFoldBlock !== 'imports') {
        accum.push('</span>', '<span class="fold-block hide-when-folded">')
      }
      inFoldBlock = 'imports'
      accum.push(line + '\n')
      ...
      if (state === 'on') {
        ...
        if (replacement) {
          ...
          accum.push('<span class="fold-block hide-when-unfolded">', replacement + '\n', '</span>')
        }
        accum.push('<span class="fold-block hide-when-folded">')
        inFoldBlock = true
      } else {

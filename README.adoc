...
*require name:* @springio/asciidoctor-extensions/code-import-extension

The code import extension provides the `import` block macro.
This macro generates code blocks by importing autodiscovered code files in one or more languages from preconfigured locations.
The location is defined first by an AsciiDoc attribute, then by the ID of the parent section (if present).
...
If the `import-java` attribute is defined, this `import` block macro will look for the Java file at the location `$\{import-java}/my/exampleproject/SomeCode.java`.
It will then create a code (i.e., source) block with the language `java` that contains the contents of this file.
The `code:` prefix in the macro target is optional.
...
For backwards compatibility, the extension supports the alternate attribute name pattern `docs-<lang>`.

A code block will be created for a matching file in each configured language.
If more than one file is found, and Asciidoctor Tabs are enabled, the code blocks will automatically be enclosed in a tabs block.

If no files are found, the extension will log a warning.

== Antora Extensions

In addition to Asciidoctor extensions, this library also provides several Antora extensions.
This section documents the auxiliary Antora extensions that are provided by this library.

=== Partial Build

*require name:* @springio/asciidoctor-extensions/partial-build-antora-extension
